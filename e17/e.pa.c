...
/*
    table entries must be sorted by name
...
S_looktbl *table;
{
    ...
    for (;;) {
	ind = U(*namptr);
	if ((*tblptr++ = isupper (ind) ? tolower (*namptr++): *namptr++)
	    == '\0')
	    break;
...
char **str;
{
    Reg1     char *cp1; /* first non-blank in str */
    Reg2     char *cp2; /* first blank or '\0' after cp1 */
    Reg3     char *cp3; /* first non-blank or '\0' after cp2 */
    char *newstr;
    static char nullstr[] = "";
