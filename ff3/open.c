...
int arena;
{
	...
	return (Ff_stream *) NULL;
    return _doffopen (path, -1, mode, arena);
}
...
int arena;
{
	...
	for (fp = ff_files; fp < &ff_files[NOFILE]; fp++) {
		...
		if (
		    ...
		    if ((ffi = ff_gfil (fp, md)) != (Ff_stream *) NULL) {
			if (     (md & F_WRITE)
			      && !(fp->fn_mode & F_WRITE)
			      && !setflock (fp->fn_fd, 1, 0))
			    return (Ff_stream *) NULL;
			fp->fn_refs++;
			fp->fn_mode |= md;
		    }
	...
	if ((ffp = ff_gfil (fp1, md)) == (Ff_stream *) NULL)
	    return (Ff_stream *) NULL;
	if (path != (char *) NULL) {
     ...
     bad:
	    ...
	    seekpos = lseek (chan, (long) 0, 1);

	if (!setflock (chan, (md & F_WRITE) != 0, 0)) {
	    if (path != (char *) NULL)
		(void) close (chan);
	    goto bad;
	}
...
int arena;
{
	...
	if ((fb = (Ff_buf *) malloc (sizeof (Ff_buf))) == (Ff_buf *) NULL)
	    break;
...
#define fb ((Ff_buf *) cp)
	...
	fb->fb_file  = (Ff_file *) NULL;
	fb->fb_bnum  = -1;
	fb->fb_count = -1;
	fb->fb_qf    = (Ff_buf *) NULL;
	fb->fb_qb    = (Ff_buf *) NULL;
	fb->fb_wflg  = 0;
	ff_flist.fr_count++;
    }
    return ff_flist.fr_count;
...
#endif  EUNICE
    if ((fb = ff_haveblk (fp, blk)) == (Ff_buf *) NULL) Block {
	Reg1 long addr;
