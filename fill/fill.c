    ...
    Default is to check access to all files and report those which can_t
...
char **argv;
{
	...
	if (curarg < numargs)
	    fprintf(stderr, ediag (
	       ...
	       "%s: при использовании nroff только stdin.\n"),
	       progname);
	else
	    feednroff();
    }
...
dohere()
{
	...
	if ( (opterrflg && opt_abort_flg) || (badfileflg && fil_abort_flg) ) {
	    fprintf(stderr,ediag("%s: not performed.\n",
				 "%s: не обрабатывается.\n"), progname);
	    exit(-2);
	}
    }
    ...
    Stop:
	    fprintf(stderr, ediag("%s: stopped.\n","%s: остановлен.\n"),
			   progname);
	    exit(-1 - output_done);
	}
	badfileflg = 0;
...
getoptions (check_only)
{
	...
	for (; *p; curargchar=p, p++) {
		...
		opterrflg = 1;
		if (show_errs_flg) {
		    fprintf(stderr,ediag ("%s: option error: -%s\n",
					 "%s: оШибоЧный клЮЧ: -%s\n"),
			progname,curargchar);
		}
		break;
...
/*                f = faccess(curargchar,"r");  uptovax WG */
		...
		if (stat(curargchar, &scratch) == -1) {
		    if (show_errs_flg) {
			fprintf(stderr,ediag ("%s: can't find %s\n",
					      "%s: %s не найден\n"),
			    progname,curargchar);
		    }
		}
		    ...
		    if (show_errs_flg) {
			fprintf(stderr,ediag ("%s: not allowed to read %s\n",
					      "%s: нельзя проЧитать %s\n"),
			    progname,curargchar);
		    }
		}
		badfileflg = 1;
...
filter()
{
	...
	fprintf(stderr,ediag ("%c%s: start typing.\n",
			      "%c%s: набирайте.\n"),
			      BELL,progname);
    doit();
    if (input != stdin)
...
doit()
{
	...
	for (;;) {                      /* treat blank lines at beginning */
	    ...
	    if (!isprint(c) && !isrus8(c) && word[lastword].nchars == 0)
		putchar(c);
	    else
		break;
	}
...
getword()
{
	...
	if ( isprint(c) || isrus8 (c) ) {
		...
		*cp++ = c;
		...
		fprintf(stderr, ediag (
		  "%s: \\b past beg of word ignored on input line %d.\n",
		  "%s: \\b после наЧала слова пропуЩен в %d строке.\n"),
		  progname,Inline);
		bksperr = 1;
		c = getch();
		break;
	...
	if (cp >= &inbuf[MAXINBUF-1]) {
	    fprintf(stderr,ediag ("%s: line overflow on input line %d!\n",
				"%s: переполнение строки в %d строке!\n"),
		progname,Inline);
	    break;
	}
    }
...
addword(wordindex)
{
	...
	if (o_nwords == 0)
		...
		space = 2;
		     ...
		     && (   isprint(lwp->brkchar) || isrus8 (lwp->brkchar)
			 || lwp->nextchar == '\n'
			 || lwp->nextchar == '\f'
			)
		     && wp->firstchar != '-'
...
getch()
{
	...
	if (  isspace(c)
	   ...
	   || c == EOF
	    ...
	    fprintf(stderr,ediag ("%s: \"\\%o\" char ignored.\n",
				  "%s: символ \"\\%o\" пропуЩен.\n"),
		    progname,c);
    }
}
