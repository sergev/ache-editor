...
int chan;
{
    ...
    if (mode & LA_NEW)
	return la_ffopen ((Ff_stream *) NULL, plas, (La_bytepos) 0);
...
La_bytepos pos;
{
	...
	if ((regi = la_nbufs - ff_flist.fr_count) > 0)
	    ff_alloc (regi, 0);
	unlink (la_cfile);
	if ((regi = open (la_cfile, O_RDWR|O_CREAT|O_EXCL, 0600)) < 0)
	    goto NoOpen;
	if ((la_chgffs = ff_fdopen (regi, 1, 0)) == (Ff_stream *) NULL) {
	    (void) close (regi);
	    goto NoOpen;
	}
	if ((la_chglas = la_open ((char *) NULL, "",
				  &chglas, (La_bytepos) 0, la_chgffs)
	       ) == NULL
	   ) {
	    ff_close (la_chgffs);
	NoOpen:
 ...
 bad:       if (nlas->la_sflags & LA_ALLOCED)
	...
	if (tlaf->la_nlines > 0) {
	    if (la_parse ((Ff_stream *) NULL, (La_bytepos) 0, &ffsd, &ffsd,
...
Reg1 La_stream *plas;
{
    ...
    (void) fill ((char *) plas, (unsigned) sizeof *plas, '\0');
    return plas;
}
...
char          *buf;
{
 ...
 makefsd:
	    ...
	    if ((cfsd = (La_fsd *) malloc ((unsigned) (LA_FSDSIZE + nfsb)))
		== NULL) {
		la_errno = LA_NOMEM;
		goto err;
	    }
	    cfsd->fsdforw = (La_fsd *) NULL;
	    cfsd->fsdback = *lfsd;
	    cfsd->fsdpos = seekpos;
	    seekpos += fsdchcnt;
	    cfsd->fsdfile = plaf;
		...
		plaf->la_fsrefs++;
	    (void) move (fsb.bytes.b, cfsd->fsdbytes, (unsigned) nfsb);
	    if (*ffsd != (La_fsd *) NULL)
		(*lfsd)->fsdforw = cfsd;
	    else
		*ffsd = cfsd;
